...
namespace ALLOCSIZE { const int size=10000; } // Size of memory block.
char memory[ALLOCSIZE::size]; // storage for memory allocation.
char *ptr = memory; // next free position.
...
class Student_info {
    ...
    void *operator new(size_t); // operator new overloading.
    void operator delete(void *); // operator delete oveloading.
};

void *Student_info::operator new(size_t size) // operator new overloading.
{
    if (memory + ALLOCSIZE::size - ptr >= size) /* it fits */
    {
        ptr += size;
        return (void*)(ptr - size);/* old p */
    } 
    else { return NULL; } /* not enough space */
}

void Student_info::operator delete(void *p) // operator delete oveloading.
{
    if (p >= memory && p < memory + ALLOCSIZE::size)
    { 
        ptr = (char*)p;
    }
}
